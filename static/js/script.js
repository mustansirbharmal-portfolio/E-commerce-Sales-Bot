document.addEventListener('DOMContentLoaded', () => {
    ...
    let product_id = localStorage.getItem('product_Id'); // Retrieve stored campaign ID
    ...
    if (msg.length === 0) {
        ...
        appendMessage(`Please Enter your Product Id`, 'bot-message', false);
    }
    else {
    ...
    function sendUserMessage(message) {
        // Store the product Id if it's the first user message
        if (msg.length === 1 && !product_id) {
           // Send the campaign ID to the backend to store it in the session
        fetch('/store_product_id', {
            method: 'POST',
            headers: {
        ...
        .then(response => {
            ...
            appendMessage(` Product Id stored successfully`, 'bot-message', true);
        ...
        .catch(error => {
            ...
            fetch('/ask', {
                ...
                body: JSON.stringify({ query: message, product_id: localStorage.getItem('product_Id') })
            })
